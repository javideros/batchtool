...
const chunkPartitionSchema = z.object({
  partitionerClass: z.string()
    .trim()
    .min(1, "Partitioner class required")
    .regex(
      /^([a-z][a-z0-9_]*\.)+([A-Z][A-Za-z0-9_]*)$/,
      "Must be a valid Java package and class name, e.g. com.example.MyPartitioner"
    ),
});

function ChunkPartitionScreen({ step, stepItemId }: { step: number; stepItemId: string }) {
  ...
  const handleSubmit = (data: any) => {
    setStepItems(
      stepItems.map(si =>
        si.id === stepItemId ? { ...si, partitionerClass: data.partitionerClass } : si
      )
    );
    handleNext(data);
  };
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg w-full mx-auto">
      <h2 className="text-lg font-semibold mb-4 text-center">Chunk Partitioner</h2>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)}>
          <FormField
            ...
            render={({ field, fieldState }) => (
              <div>
                <label className="block text-sm font-medium mb-1">Partitioner Class</label>
